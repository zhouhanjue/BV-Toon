...
面板（3D 视图 → N → 卡渲）：
    ...
    其他设置      添加泛光（+移除）/ 设置目影 / 添加腮红 / 烧录署名 / 使用条款
...
bl_info = {
    ...
    "description": "MMD 模型一键卡渲：原生读取材质、一键套用、边缘预览、目影、腮红、泛光、强制署名水印。",
    "category": "Material",
    "support": "COMMUNITY",
}
...
class BVTOON_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = box2.row(align=True)
        row.operator("bvtoon.add_blush", text="添加腮红（只给脸）", icon="OVERLAY")
        row.operator("bvtoon.set_face_material", text="脸=当前材质", icon="EYEDROPPER")
        row.operator("bvtoon.clear_face_material", text="", icon="X")
        row = box2.row(align=True)
        row.label(text="脸材质：%s" % context.scene.get("bv_face_materials", "未识别"),
                  icon="INFO")
        if context.scene.get("bv_blush_mechanism"):
            row = box2.row(align=True)
            row.label(text="腮红：%s" % context.scene["bv_blush_mechanism"][:40],
                      icon="OVERLAY")
        note = context.scene.get("bv_face_note", "")
        if "⚠" in note:          # 认脸是"猜"的、而且猜得很可疑：直接挂出来让人改
            row = box2.row(align=True)
            row.alert = True
            row.label(text=note.split("；⚠")[-1][:60], icon="ERROR")
        row = box2.row(align=True)
...
class BVTOON_OT_set_face_material(bpy.types.Operator):
    """手动指定脸材质：用当前激活的材质槽（认不出脸的模型走这条）"""

    bl_idname = "bvtoon.set_face_material"
    bl_label = "脸=当前材质"

    def execute(self, context):
        active = context.object.active_material if context.object else None
        if active is None or active.node_tree is None \
                or active.node_tree.nodes.get("bv_shading") is None:
            self.report({"ERROR"}, "先在材质槽里选中一个卡渲过的材质（物体模式）")
            return {"CANCELLED"}
        context.scene["bv_face_material"] = active.name
        context.scene["bv_face_materials"] = active.name
        context.scene["bv_blush_mechanism"] = "手动指定"
        self.report({"INFO"}, "已把「%s」指定为脸材质，再点「添加腮红（只给脸）」生效"
                    % active.name)
        print("[BV-Toon] 脸材质手动指定 -> %s" % active.name)
        return {"FINISHED"}


class BVTOON_OT_clear_face_material(bpy.types.Operator):
    """清掉手动指定的脸材质，回到自动识别"""

    bl_idname = "bvtoon.clear_face_material"
    bl_label = "清除脸材质指定"

    def execute(self, context):
        for key in ("bv_face_material", "bv_face_materials"):
            if key in context.scene:
                del context.scene[key]
        self.report({"INFO"}, "已清除指定，下次按自动识别")
        return {"FINISHED"}


class BVTOON_OT_add_blush(bpy.types.Operator):
    """只给「脸」加腮红：先认出脸部材质，再写进它的卡渲组，其余材质上的腮红清零"""
    ...
    def execute(self, context):
        ...
        materials = target_materials(context) or [
            slot.material for obj in objects
            for slot in obj.material_slots if slot.material is not None]
        if not materials:
            self.report({"ERROR"}, "先选中模型（物体模式）")
            return {"CANCELLED"}
        shadable = [mat for mat in materials if mat.node_tree is not None
                    and mat.node_tree.nodes.get("bv_shading") is not None]
        if not shadable:
            self.report({"ERROR"}, "这些材质还没卡渲过，请先点「一键卡渲」")
            return {"CANCELLED"}

        # 模式＝替换单个材质时听用户的：就用当前激活的那个材质槽，不看名字
        # 面板上「脸=当前材质」指定的也优先（认不出脸的模型走这条）
        face, why, manual = [], "", False
        chosen = context.scene.get("bv_face_material", "")
        picked = next((mat for mat in shadable if mat.name == chosen), None) if chosen else None
        if picked is not None:
            face, why, manual = [picked], "面板手动指定的脸材质（%s）" % chosen, True
        elif getattr(context.scene, "bv_mode", "") == "REPLACE_SINGLE":
            active = context.object.active_material if context.object else None
            if active is not None and active in shadable:
                face = [active]
                why = "面板模式=替换单个材质，用的是当前激活的材质槽"
                manual = True
        if not face:
            face, why = bv_materials.face_materials(shadable, objects)
        if not face:
            self.report({"WARNING"},
                        "没认出哪个材质是脸：%s（把脸部材质改名成含「颜」或「face」，"
                        "或者选中脸材质后点面板上的「脸=当前材质」）" % why)
            print("[BV-Toon] 腮红跳过：%s" % why)
            return {"CANCELLED"}

        done = sum(1 for mat in face if bv_materials.set_blush(mat))
        cleared = 0
        if not manual:
            for mat in shadable:                    # 以前"全都加"留下的，顺手清掉
                if mat in face:
                    continue
                if bv_materials.blush_amount(mat):  # 非 0 才算要清
                    if bv_materials.set_blush(mat, 0.0):
                        cleared += 1
        # 优先用**模型自己带的腮红贴图**（照れ）当遮罩 —— 作者画的形状比我们猜的阈值准
        texture, why = bv_materials.blush_texture(shadable)
        masked = sum(1 for mat in face if bv_materials.wire_blush_mask(mat, texture)) \
            if texture is not None else 0
        if texture is None:
            for mat in face:                        # 别留着上一次接的贴图
                bv_materials.clear_blush_mask(mat)
        # 脸颊圈：按**脸材质自己的几何**把腮红收到两颊；认不出来就退回整脸（照样只加在
        # 这块脸材质上，不会糊到身体）。**不管用不用贴图遮罩都要摆好** —— 贴图遮罩是
        # "替换"内置遮罩的，哪天贴图被摘掉（用户手改 / 换模型），摆好的脸颊圈立刻顶上，
        # 不然只剩一层保底，实测那种情况下只有 3 个像素看得出变化。
        placed = [bv_materials.place_blush(mat, objects) for mat in face]
        hit = next(((ok, why) for ok, why in placed if ok), None)
        region = "脸颊（%s）" % hit[1] if hit else \
            "整脸（认不出脸颊：%s）" % (placed[0][1] if placed else "没有材质")
        for ok, why in placed:
            print("[BV-Toon] 脸颊圈 %s：%s" % ("ok" if ok else "退回整脸", why))
        mechanism = ("用模型自带的腮红贴图「%s」当遮罩（%s）" % (texture.name, why)
                     if masked else "着色器腮红，位置=%s" % region)
        context.scene["bv_face_materials"] = "、".join(mat.name for mat in face)
        context.scene["bv_face_note"] = why
        context.scene["bv_blush_mechanism"] = mechanism
        message = "已给 %d 个脸部材质加上腮红：%s" % (done, mechanism)
        if "⚠" in why:                        # 猜得很可疑：报告里也提醒一次
            message += "；⚠ %s" % why.split("；⚠")[-1]
        if cleared:
            message += "；顺手清掉 %d 个非脸部材质上的腮红" % cleared
        self.report({"INFO"}, message)
        print("[BV-Toon] 腮红 -> %d 个脸部材质；%s；贴图遮罩接了 %d 个；清零 %d 个非脸部材质"
              % (done, mechanism, masked, cleared))
        return {"FINISHED"}
...
CLASSES = (
    ...
    BVTOON_OT_add_blush,
    BVTOON_OT_set_face_material,
    BVTOON_OT_clear_face_material,

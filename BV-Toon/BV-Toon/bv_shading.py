...
INPUTS = (
    ...
    ("腮红范围", "NodeSocketFloat", 0.32),
    ("腮红过渡", "NodeSocketFloat", 0.30),
...
def build_shading_group():
    ...
    # ---- 腮红：只在暗部露出来的一层颜色 --------------------------------
    # 看已经算好的颜色有多亮（不用 ShaderToRGB、也不混着色器）：
    # 亮度低于"腮红范围"的地方才上色，正好落在卡渲的阴影里。
    grey = nodes.new("ShaderNodeRGBToBW")
    grey.location = (1100, -200)
    links.new(ringed_out, grey.inputs["Color"])
    gap = math_node(nodes, "SUBTRACT", None, "阈值 - 亮度", (1280, -200),
                    clamp=True)
    links.new(source.outputs["腮红范围"], gap.inputs[0])
    links.new(grey.outputs["Val"], gap.inputs[1])
    blush_soft = math_node(nodes, "MAXIMUM", 0.02, "腮红过渡", (1280, -340))
    links.new(source.outputs["腮红过渡"], blush_soft.inputs[0])
    blush_mask = math_node(nodes, "DIVIDE", None, "腮红遮罩", (1460, -200),
                           clamp=True)
    links.new(gap.outputs[0], blush_mask.inputs[0])
    links.new(blush_soft.outputs[0], blush_mask.inputs[1])
    ...
    blushed, blush_fac, blush_a, blush_b, blushed_out = mix(
        nodes, "MIX", 0.0, "上腮红", (1640, 40))
    links.new(ringed_out, blush_a)
    links.new(source.outputs["腮红色"], blush_b)
    links.new(blush_amount.outputs[0], blush_fac)

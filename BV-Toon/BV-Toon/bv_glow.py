...
#: Glare 质量。**HIGH 换 MEDIUM 是性能优化**（2026-09-20 实测，3840×2160、胡桃）：
#: 两层 Glare 是泛光里最贵的一段，4.5 的 CPU 合成器上 HIGH→MEDIUM 省 0.6 秒、
#: HIGH→LOW 省 0.8 秒，而画面差异只有 0.016（MEDIUM）/ 0.028（LOW）的最大亮度 ——
#: 泛光本身是大半径低频的东西，金字塔层数少一层看不出来。5.2 上三者几乎一样快。
GLOW_QUALITY = "MEDIUM"
GLOW_MODE = "BLOOM"       # 4.4+ 的灯光型 glare

#: 泛光**在几分之一的画布上算**（1.0 = 全分辨率，也就是默认不降）。
#: 2026-09-20 加，本来是为了性能；量完发现**只有 CPU 合成器上才划算**：
#: GPU 合成器上泛光的开销是"每个节点一次调度"的固定成本，降到 1/2、1/4 画布
#: 一点都不省（4K 实测 +0.49 s vs +0.47 s），却会让剪影外的光往外摊一点
#: （角色上的泛光 -16%、剪影外不变）。所以默认 **1.0：画面上一个像素都不动**。
#: 万一哪天只能在 CPU 合成器上渲（显卡用不了），改成 0.5 能省一大半 ——
#: 实测（3840×2160、胡桃、同一台机器、预热后交替取中位）：
#:
#: ==========  ==============  ==============  ==============
#: 画布        4.5 CPU 合成器   4.5 GPU 合成器   5.2（默认 GPU）
#: ==========  ==============  ==============  ==============
#: 1.0        泛光 +3.38 s    泛光 +0.47 s    泛光 +0.65 s
#: 0.5        泛光 +1.13 s    泛光 +0.49 s    泛光 +0.65 s
#: ==========  ==============  ==============  ==============
GLOW_CANVAS = 1.0
#: 4.5 的合成器**默认走 CPU**：同一个 4K 场景，泛光增量 CPU 3.01 秒 / GPU 1.04 秒，
#: 而且两张图逐像素一致（最大差 0.004）。5.2 默认已经是 GPU，不用管。
#: 用户的设置会被记住，摘掉泛光时还回去（见 :func:`_fast_compositor`）。
SCENE_DEVICE_FLAG = "bvtoon_glow_device"
...
def _scale_node(nodes, label, title, location, factor):
    """合成器里的「缩放」节点。4.5 上模式是属性（space）+ X/Y 插座；
    5.x 上全是菜单插座（Type = Relative）。两边都要能设上，否则泛光会画在全分辨率上。"""
    node = nodes.new("CompositorNodeScale")
    node.name = "%s_%s" % (GLOW_PREFIX, label)
    node.label = title
    node.location = location
    if _has(node, "space"):
        _set(node, "space", "RELATIVE")
    else:
    ...
    for name in ("X", "Y"):
        ...
        try:
            ...
            print("[BV-Toon] 缩放节点 %s 设不上：%s" % (name, error))
    return node
...
def glow_group():
    ...
    # 降分辨率算泛光（见 GLOW_CANVAS）：前段全部在 GLOW_CANVAS 倍的画布上做，
    # 只有最后"加到画面上"那一步回到全分辨率 —— 画面本身不经过降采样，不会被糊。
    # GLOW_CANVAS >= 1 就等于"不降"（想回到逐像素一样的旧画面时用）。
    ...
    add.label = "加到画面上"
    add.location = (700, 0)
    # 这一步必须是**原图**（全分辨率），所以走 source 而不是降采样后的那份
...
def _fast_compositor(scene):
    ...
    实测（3840×2160、胡桃、同一台机器）：泛光在 CPU 合成器上要多花 3.01 秒，
    GPU 只要 1.04 秒，而且两张图逐像素一致（最大亮度差 0.004）。
    用户原来的设置记在 :data:`SCENE_DEVICE_FLAG` 里，摘掉泛光时还回去。
    """
    ...
    try:
        ...
        print("[BV-Toon] 合成器切 GPU 失败（画面不受影响）：%s" % (error,))
    return None
...
def insert_glow(scene):
    ...
    try:
        ...
        pass
    # 顺手把合成器换到 GPU（4.5 默认 CPU，泛光 4K 下要多花 2 秒）

...
git init
git add .
git commit -m "BV-Toon 1.8.0: one-click MMD toon shading + integrity-check watermark"
git branch -M main
...
**Releases → Draft a new release → tag `v1.8.0`** → paste the contents of
[`RELEASE-1.8.0.md`](RELEASE-1.8.0.en.md) into the description → attach
`dist\BV-Toon-1.8.0.zip` (its top level is `BV-Toon/`, so users can install it directly).
...
* License: **GNU GPL v3** (plus Section 7 attribution terms) — ✅ use, modify and redistribute freely (incl. commercial) ｜ ✅ copies and modified versions must stay GPL v3 ｜ ✅ the credit `BVToon / @BVan` must be preserved. Full text in [`LICENSE`](LICENSE).
...
| Release notes | `RELEASE-1.8.0.md` | `RELEASE-1.8.0.en.md` | `RELEASE-1.8.0.ja.md` |
